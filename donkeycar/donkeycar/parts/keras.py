...
def default_n_linear(num_outputs, input_shape=(120, 160, 3), roi_crop=(0, 0)):
    ...
    slam_in = Input(shape=(300,4), name='slam_int')


    # First branch for image
    x = img_in
    ...
    for i in range(num_outputs):
        x_outputs.append(Dense(1, activation='linear', name='x_n_outputs' + str(i))(x))
        
    x_model = Model(inputs=[img_in], outputs=x_outputs)
    
    # Second branch for slam points
    y = slam_in
    ...
    for i in range(num_outputs):
        y_outputs.append(Dense(1, activation="linear", name='y_n_outputs' + str(i))(y))
    
    y_model = Model(inputs=[slam_in], outputs=y_outputs)
    ...
    model = Model(inputs=[x_model.input, y_model.input], outputs=outputs)
